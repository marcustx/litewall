...
{
    public function testIsValidRouteForEachHand()
    {
        $validator = new RouteValidator($this->getConfig());

        $sut = [ "A1R", "A2L", "A3M" ];

        $this->assertSame(
            true,
            $validator->isValidRoute($sut)
        );
    }

    public function testIsInValidHoldsRoute()
    {
        $validator = new RouteValidator($this->getConfig());
        ...
        $this->expectExceptionMessage("Hold position as not found in led_id_map");

        $validator->isValidRoute($sut);
    }

    public function testIsInValidRouteHand()
    {
        $validator = new RouteValidator($this->getConfig());
        ...
        $this->expectExceptionMessage("Z is not a valid hand char.  Must be L, R or M");

        $validator->isValidRoute($sut);
    }

    public function testIsInValidRouteHandLength()
    {
        $validator = new RouteValidator($this->getConfig());
        ...
        $this->expectExceptionMessage("Hold position A1L not found in led_id_map");
